...
class AppRunner {
    ...
    private fun loadInitialData() {
        ...
        if (
            userUseCases.getAllUsers().isEmpty()
            || categoryUseCases.getAllCategories().isEmpty()
            || productUseCases.getAllProducts().isEmpty()
        ) {
    ...
    private fun runCategoryOperations() {
        ...
        // Update category
        val categoryToUpdate = addedCategory.copy(
            name = "Outdoor & Sports",
            description = "Gear for outdoor activities and sports"
        )
    ...
    private fun updateExistingUser(user: User): User? {
        val updatedGeo = user.address.geo.copy(
            lat = "34.0522",
            lng = "-118.2437"
        ) // Los Angeles
        val updatedAddress = user.address.copy(
            street = "456 Oak Ave",
            city = "Los Angeles",
            zipcode = "90012",
            geo = updatedGeo
